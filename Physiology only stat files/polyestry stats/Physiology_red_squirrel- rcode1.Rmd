...
squirrel <- fread('sphys_ra528.csv',data.table = F)
...
analyze_yearly_trends <- function(m1, lag=1, cutoff_estradiol=1500,
                                  cutoff_progesterone=300, 
                                  dur_estradiol=0,dur_progesterone=1){
  ### Detect events 
  ...
  for(i in 1:length(event)){
    ##variable to tell if progesterone was above x for a duration of y
    proge_sustain <- all(proge_max[i:(i+dur_progesterone)]==1)
    estra_sustain <- all(estra_max[i:i+dur_estradiol]==1)
##    if(estra_max[i]==1 & proge_max[i+lag]==1){-changed progto 2
    if(estra_max[i]==1 & proge_sustain  ){
      event[i] <- 1
...
##commented out because code has been updated so that lag is not used
## analyze_yearly_trends(m1=m1, lag = 2)
analyze_yearly_trends(m1=m1)
```
...
```{r, fig.align='center', fig.width=6, fig.height=6}
##analyze_yearly_trends(m1=m2, lag = 0)- lag=0 removed in the line below
analyze_yearly_trends(m1=m2)
```
